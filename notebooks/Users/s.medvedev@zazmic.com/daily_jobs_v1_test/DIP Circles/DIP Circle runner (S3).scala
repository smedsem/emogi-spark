...
val timezoneCorrection = 0 //5 * 60 * 60 * 1000 //for DIP we're getting UTC but then converting to the 'local time' - EST
...
val dayRanges = for (day <- 0 until daysCount) yield {
  ...
  (tsFrom.getMillis + timezoneCorrection, tsTo.getMillis + timezoneCorrection)
}
...
dayRanges.foreach { day =>
  ...
  print(s"Started collecting DIP CIRCLE DATA collection for the time range from ${new DateTime(day._1)} to ${new DateTime(day._2)}..")
  val result = dbutils.notebook.run("/Users/s.medvedev@zazmic.com/daily_jobs_v1_test/DIP Circles/DIP Circle data (S3)", timeoutSec, Map("dateFrom" -> day._1.toString, "dateTo" -> day._2.toString))
...
println(s"Finished processing DIP CIRCLE DATA collection from=$from to=$to, it took $durationGlobalMinutes minutes")
