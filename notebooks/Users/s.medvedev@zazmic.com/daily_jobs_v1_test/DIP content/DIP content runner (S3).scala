...
val timezoneCorrection = 5 * 60 * 60 * 1000 //for DIP we're getting UTC but then converting to the 'local time' - EST
...
dayRanges.foreach { day =>
  ...
  print(s"Started collecting DIP content stats for the time range from ${new DateTime(day._1)} to ${new DateTime(day._2)}..")
  val result = dbutils.notebook.run("/Users/s.medvedev@zazmic.com/daily_jobs_v1_test/DIP content/DIP content (S3)", timeoutSec, Map("dateFrom" -> day._1.toString, "dateTo" -> day._2.toString, "s3Folder" -> s3Folder))
...
println(s"Finished processing DIP content stats from=$from to=$to, it took $durationGlobalMinutes minutes")
