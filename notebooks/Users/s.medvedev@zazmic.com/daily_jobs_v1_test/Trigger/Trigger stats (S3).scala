...
val mxRawRdd = conxDF
  ...
  .map {row =>
    ...
    val av = extractNullableString(row, 6)
    val is_branded = av != null
    (ap, at, tr, xp, ma, new Timestamp(hourTimestamp.getMillis), is_branded)
  }
  .toDF("ap", "at", "tr", "xp", "ma", "dt", "is_branded")
...
.map { row =>
  ...
  val is_branded = row._1._5
  (row._1._4, 
   dt.getYear, dt.getMonthOfYear, dt.getDayOfMonth, dt.getHourOfDay, dt.dayOfWeek().get, 
   views, shares, 
   xplaId, trigId, appId, is_branded)
}
.toDF("date","year","month","day","hour","dow","views","shares","xpla_id","trig_id","app_id", "is_branded")
