...
dayRanges.foreach { day =>
  ...
  print(s"Started collecting ODS S3 stats for the time range from ${new DateTime(day._1)} to ${new DateTime(day._2)}..")
  val result = dbutils.notebook.run("/Users/s.medvedev@zazmic.com/daily_jobs_v1_test/ODS_stats/ODS stats (S3)", timeoutSec, Map("dateFrom" -> day._1.toString, "dateTo" -> day._2.toString, "s3Folder" -> s3Folder))
...
println(s"Finished processing ODS S3 stats from=$from to=$to, it took $durationGlobalMinutes minutes")
